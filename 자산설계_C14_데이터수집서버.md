...
[STEP 4-5 완료] 군 INT 존 장악
    │
    ▼
[STEP 5-1] C12 망 연동 서버 (192.168.92.234) — 동기화 스크립트 변조
    │
    ▼
[STEP 5-2] C14 데이터 수집 서버 (192.168.92.234) — 핵심 데이터 변조
    │
...
TARGET="http://192.168.92.234:8000"
...
echo "호스트: data.c4i.local (192.168.92.234)"
echo "=========================================="
...
|---|------|--------|--------|------|----------|------|------|
| 1 | WAN→LAN | 192.168.92.254 (관리자 PC) | 192.168.92.234 | 22 | TCP | ALLOW | SSH 관리 접근 |
| 2 | WAN→LAN | 192.168.92.234 (INT 포털) | 192.168.92.234 | 443 | TCP | ALLOW | INT→릴레이 |
| 3 | LAN→LAN | 192.168.92.0/24 | 192.168.92.0/24 | * | * | ALLOW | C4I 내부 전체 허용 |
| 4 | LAN→WAN | 192.168.92.234 | 192.168.92.234 | 8080 | TCP | ALLOW | 릴레이→INT |
...
```bash
192.168.92.234   gw.c4i.local
192.168.92.234  relay.c4i.local
192.168.92.234  cop.c4i.local
192.168.92.234  data.c4i.local
192.168.92.234  summary.c4i.local
(테스트 미할당)  ops-pc-1.c4i.local
(테스트 미할당)  ops-pc-2.c4i.local
(테스트 미할당)  ops-pc-3.c4i.local
(테스트 미할당)  ops-pc-4.c4i.local
192.168.92.234  ops-pc-5.c4i.local
```
...
| 자산 | IP | 포트 | 프로토콜 | 서비스 |
|------|-----|------|----------|--------|
| C14 | 192.168.92.234 | 8000 | TCP | FastAPI (이벤트 API) |
| C14 | 192.168.92.234 | 5432 | TCP | PostgreSQL (로컬) |
