...
```
요청: GET http://192.168.92.214:5000/api/status
인증: 없음
...
```
요청: GET http://192.168.92.214:5000/api/config
인증: 없음
...
```
요청: POST http://192.168.92.214:5000/api/set_range
...
curl -X POST http://192.168.92.214:5000/api/set_range \
...
```
요청: POST http://192.168.92.214:5000/api/inject
Content-Type: application/json
...
```
요청: GET http://192.168.92.214:5000/api/history?sensor=temperature&count=10
인증: 없음
...
```
요청: GET http://192.168.92.214:5000/api/health
...
```
[1단계] 정찰
    $ curl http://192.168.92.214:5000/api/config
...
[3단계] 값 주입 (적극적 공격)
    $ curl -X POST http://192.168.92.214:5000/api/inject \
        -H "Content-Type: application/json" \
...
# 대상: 192.168.92.214 (Ubuntu 22.04)
...
echo "=== 설치 완료 ==="
echo "PLC API: http://192.168.92.214:5000"
echo "상태 확인: curl http://192.168.92.214:5000/api/health"
```
...
tail -F "$LOG_FILE" | while read line; do
    ...
    if [ "$SRC_IP" != "192.168.92.214" ] && [ "$SRC_IP" != "127.0.0.1" ]; then
