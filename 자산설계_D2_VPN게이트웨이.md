...
D2 VPN 게이트웨이는 발도리아국 국방부의 원격접속 VPN 서버이다. 외부 근무자나 협력기관 직원이 군 내부 자원에 안전하게 접근할 수 있도록 OpenVPN Access Server를 운영한다. VPN 인증 후에는 패치관리 전용 서브넷(192.168.120.0/24)에만 접근이 허용된다.
...
```
[AI 어시스턴트 (192.168.100.13)] — 군 VPN 자격증명 추출 완료
    │
    │  GOV20190847 / 20190847890312
    │  vpn.mnd.valdoria.mil (211.57.64.11)
    │
    ▼
[★ D2 VPN 게이트웨이 (211.57.64.11)] ← STEP 4-1: VPN 로그인
    │
    │  인증 성공 → 클라이언트 IP: 172.20.100.100~200 중 할당
    │  라우팅: 192.168.120.0/24만 허용
    │
    ├──▶ [패치관리서버 (192.168.120.10)] ← STEP 4-2: 공급망 공격
    │       │
    │       ▼
    │    [군 업무용 PC (192.168.110.31~35)] ← 악성 패치 실행
    │
    └──▶ [D3 자료교환체계 (211.57.64.12)] ← STEP 4-3: 문서 탈취
...
| 인터페이스 | IP | 용도 |
|------------|-----|------|
| ens192 | 211.57.64.11/24 | DMZ 인터페이스 (VPN 리스닝) |
| tun0 | 172.20.100.1/24 | VPN 터널 인터페이스 |
...
# VPN 데몬 설정
/usr/local/openvpn_as/scripts/sacli ConfigPut "vpn.daemon.0.listen.0.0" "211.57.64.11"
...
# TCP 폴백 (방화벽 우회용)
/usr/local/openvpn_as/scripts/sacli ConfigPut "vpn.daemon.0.listen.1.0" "211.57.64.11"
...
# 클라이언트 IP 풀
/usr/local/openvpn_as/scripts/sacli ConfigPut "vpn.server.dhcp_option.address.1" "172.20.100.0"
/usr/local/openvpn_as/scripts/sacli ConfigPut "vpn.server.dhcp_option.netmask.1" "255.255.255.0"
/usr/local/openvpn_as/scripts/sacli ConfigPut "vpn.server.dhcp_option.pool.start" "172.20.100.100"
/usr/local/openvpn_as/scripts/sacli ConfigPut "vpn.server.dhcp_option.pool.end" "172.20.100.200"
...
# 라우팅 설정: 패치관리 서브넷만 허용
/usr/local/openvpn_as/scripts/sacli ConfigPut "vpn.server.routing.private_network.0" "192.168.120.0/24"
# 군 INT (192.168.110.0/24) 라우팅은 의도적으로 미설정
...
{
  "Default": {
    "vpn.daemon.0.listen.0.0": "211.57.64.11",
    "vpn.daemon.0.listen.0.1": "1194",
    "vpn.daemon.0.listen.0.2": "udp",
    "vpn.daemon.0.listen.1.0": "211.57.64.11",
    "vpn.daemon.0.listen.1.1": "443",
    "vpn.daemon.0.listen.1.2": "tcp",
    "vpn.server.dhcp_option.dns.0": "192.168.120.1",
    "vpn.server.routing.private_network.0": "192.168.120.0/24",
...
**핵심 설계 포인트**: VPN 인증 성공 후 클라이언트는 **192.168.120.0/24 (패치관리 서브넷)에만** 접근할 수 있다. 군 INT(192.168.110.0/24)로의 직접 접근은 차단된다.

```bash
# iptables 규칙 (VPN 서버에서)
# VPN 터널(tun0) → 패치관리 서브넷만 허용
iptables -A FORWARD -i tun0 -d 192.168.120.0/24 -j ACCEPT
iptables -A FORWARD -i tun0 -d 192.168.110.0/24 -j DROP    # 군 INT 직접 차단
iptables -A FORWARD -i tun0 -d 192.168.130.0/24 -j DROP    # C4I 직접 차단
iptables -A FORWARD -i tun0 -j DROP                          # 그 외 전부 차단

# NAT (VPN 클라이언트 → 패치관리 서브넷)
iptables -t nat -A POSTROUTING -s 172.20.100.0/24 -d 192.168.120.0/24 -o ens192 -j MASQUERADE

# 라우팅 확인 (클라이언트 측)
# VPN 접속 후:
# $ ip route
# 172.20.100.0/24 dev tun0 scope link
# 192.168.120.0/24 via 172.20.100.1 dev tun0
```
...
|---|------|--------|--------|------|----------|------|------|
| 1 | IN | 172.20.100.0/24 | 192.168.120.10 | 80, 8080 | TCP | ALLOW | VPN→패치관리서버 |
| 2 | IN | 172.20.100.0/24 | 192.168.110.0/24 | ANY | ANY | **DENY** | VPN→군INT 직접 차단 |
| 3 | IN | 172.20.100.0/24 | 192.168.130.0/24 | ANY | ANY | **DENY** | VPN→C4I 직접 차단 |
| 4 | IN | 172.20.100.0/24 | ANY | ANY | ANY | **DENY** | 기본 차단 |
| 5 | IN | 211.57.64.0/24 | 192.168.110.0/24 | 특정 포트 | TCP | ALLOW | DMZ→INT 서비스 |
...
# 3. 접속 확인
ip addr show tun0
# 예상: inet 172.20.100.xxx/24

ip route | grep 192.168.120
# 예상: 192.168.120.0/24 via 172.20.100.1 dev tun0

# 4. 패치관리서버 접근 확인
curl -s http://192.168.120.10/
# 예상: 패치관리서버 웹 페이지 응답

# 5. 군 INT 직접 접근 시도 (실패 예상)
curl -s --connect-timeout 3 http://192.168.110.10/
# 예상: timeout (라우팅 차단)
```
...
# 초기 설정 (설치 시 자동 실행됨)
# Admin URL: https://211.57.64.11:943/admin
# Client URL: https://211.57.64.11:943/
...
2026-03-27 02:34:17 [OVPN] GOV20190847/203.238.140.xxx: Assigned IP 172.20.100.105

# 비업무 시간 접속 (새벽 2시)
# 이전에 GOV20190847 접속 이력 없음 → 최초 로그인 알림 필요

# VPN 접속 직후 패치관리 서브넷 스캔
2026-03-27 02:35:01 [iptables] VPN-ALLOWED: IN=tun0 SRC=172.20.100.105 DST=192.168.120.10
2026-03-27 02:35:02 [iptables] VPN-BLOCKED: IN=tun0 SRC=172.20.100.105 DST=192.168.110.10
```
...
| VPN 접속 후 192.168.110.0/24 접근 시도(차단) | Medium | T1046 (Network Service Discovery) |
...
| D2 VPN 관리 | 211.57.64.11 | 943 | TCP | OpenVPN AS Admin UI |
...
```env
# ── D2 VPN 게이트웨이 (211.57.64.11) ───────────────────────
MIL_DOMAIN_EXT=mnd.valdoria.mil
VPN_HOST=211.57.64.11
VPN_PORT_UDP=1194
VPN_PORT_TCP=443
VPN_ADMIN_PORT=943
VPN_CLIENT_POOL=172.20.100.100-200
VPN_ROUTE_ALLOWED=192.168.120.0/24
```
...
| 2 | D2 VPN 로그인 | OpenVPN 클라이언트 + GOV20190847 | 172.20.100.x 할당 |
| 3 | D2 패치관리 접근 | VPN 후 `curl http://192.168.120.10` | 200 OK |
| 4 | D2 군INT 차단 | VPN 후 `curl http://192.168.110.10` | Timeout |
