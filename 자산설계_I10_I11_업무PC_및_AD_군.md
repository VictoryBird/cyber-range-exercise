...
| IP 범위 | 192.168.92.249~35 (업무용 PC), 192.168.92.254 (관리자 PC) |
| OS | Windows 10/11 Pro 22H2 |
| AD 도메인 | corp.mnd.local |
| DC | MIL-DC01 (192.168.92.230) |
...
[STEP 4-2 결과] 패치 서버에서 악성 업데이트 배포
    ...
    │
    ▼
[PC-1~5 감염] 192.168.92.249~35 — Havoc C2 세션 확립
    │
    ├── Mimikatz 실행
    ...
    │
    ├── PsExec 측면 이동 → 관리자 PC (192.168.92.254)
    │   └── mil_admin 자격증명 사용
    │
    ▼
[관리자 PC] 192.168.92.254
    │
    ├── DCSync 공격
    ...
    └── Golden Ticket 생성 → 도메인 전체 장악
        │
        ▼
    [STEP 5] C4I 진입 (OPNSense-7 → 192.168.92.0/24)
```
...
|--------|---------|---------|--------|-----------|-----|
| 업무용 PC-1 | MIL-PC01 | 192.168.92.249 | /24 | 192.168.92.230 | 192.168.92.230 |
| 업무용 PC-2 | MIL-PC02 | 192.168.92.250 | /24 | 192.168.92.230 | 192.168.92.230 |
| 업무용 PC-3 | MIL-PC03 | 192.168.92.251 | /24 | 192.168.92.230 | 192.168.92.230 |
| 업무용 PC-4 | MIL-PC04 | 192.168.92.252 | /24 | 192.168.92.230 | 192.168.92.230 |
| 업무용 PC-5 | MIL-PC05 | 192.168.92.253 | /24 | 192.168.92.230 | 192.168.92.230 |
| 관리자 PC | MIL-ADMIN01 | 192.168.92.254 | /24 | 192.168.92.230 | 192.168.92.230 |
...
| DC 호스트명 | MIL-DC01 |
| DC IP | 192.168.92.230 |
| FSMO 역할 | 모두 MIL-DC01 보유 (단일 DC) |

> **참고:** AD DC(192.168.92.230)는 본 문서 범위에서 제외하며, I10/I11 PC에서의 DC 연동 관점에서만 기술한다. DC 상세 설계가 필요한 경우 별도 문서로 작성한다.
...
│   ├── OU=업무용PC
│   │   ├── PC: MIL-PC01 (192.168.92.249)
│   │   ├── PC: MIL-PC02 (192.168.92.250)
│   │   ├── PC: MIL-PC03 (192.168.92.251)
│   │   ├── PC: MIL-PC04 (192.168.92.252)
│   │   └── PC: MIL-PC05 (192.168.92.253)
│   │
│   └── OU=관리자PC
│       └── PC: MIL-ADMIN01 (192.168.92.254)
│
├── OU=서버 (Servers)
│   └── DC: MIL-DC01 (192.168.92.230)
│
...
curl -u admin:admin123 \
    ...
    http://192.168.92.229/admin/upload.php
```
...
psexec \\192.168.92.254 -u corp\mil_admin -p MilAdmin2026! cmd.exe
...
│  #  │     출발지         │       목적지         │   포트    │  동작   │         비고             │
├─────┼──────────────────┼────────────────────┼──────────┼────────┼─────────────────────────┤
│  1  │ 192.168.92.0/24  │ 192.168.92.229     │ 80/tcp   │ ALLOW  │ PC → 패치서버 업데이트    │
│  2  │ 192.168.92.0/24  │ 192.168.92.0/24   │ *        │ ALLOW  │ INT 내부 통신 전체 허용   │
└─────┴──────────────────┴────────────────────┴──────────┴────────┴─────────────────────────┘
```
...
│  #  │     출발지         │       목적지         │   포트    │  동작   │         비고             │
├─────┼──────────────────┼────────────────────┼──────────┼────────┼─────────────────────────┤
│  1  │ 192.168.92.254    │ 192.168.92.0/24   │ *        │ ALLOW  │ 관리자 PC → C4I 관리     │
│  2  │ 192.168.92.230    │ 192.168.92.0/24   │ 53,88,389│ ALLOW  │ DC → C4I AD 서비스       │
│  3  │ 192.168.92.0/24  │ 192.168.92.0/24   │ *        │ DENY   │ 일반 PC → C4I 차단       │
└─────┴──────────────────┴────────────────────┴──────────┴────────┴─────────────────────────┘
...
│  #  │     출발지         │       목적지         │   포트    │  동작   │         비고             │
├─────┼──────────────────┼────────────────────┼──────────┼────────┼─────────────────────────┤
│  1  │ 192.168.92.0/24  │ 192.168.92.230     │ 389,88   │ ALLOW  │ C4I → DC 인증            │
│  2  │ 192.168.92.0/24  │ 192.168.92.0/24   │ *        │ DENY   │ C4I → INT 역방향 차단     │
└─────┴──────────────────┴────────────────────┴──────────┴────────┴─────────────────────────┘
```
...
mil-dc01.corp.mnd.local.      IN  A     192.168.92.230
mil-pc01.corp.mnd.local.      IN  A     192.168.92.249
mil-pc02.corp.mnd.local.      IN  A     192.168.92.250
mil-pc03.corp.mnd.local.      IN  A     192.168.92.251
mil-pc04.corp.mnd.local.      IN  A     192.168.92.252
mil-pc05.corp.mnd.local.      IN  A     192.168.92.253
mil-admin01.corp.mnd.local.   IN  A     192.168.92.254
